...
export const Box = class Box extends Widget {
   ...
   build() {
      ...
      procedureName.className = 'col-sm-4 text-center';
      procedureName.style.fontStyle = 'italic';
      procedureName.style.fontWeight = 'bold';
      procedureName.innerHTML = this.getProcedure();

      let date = document.createElement('div');
      date.className = 'col-sm-4 text-center';
      date.style.fontStyle = 'italic';
      date.innerHTML = 'DATE: ' + moment(data[this.getProcedure()].lastObservation.date).format('DD-MM-YYYY');

      let time = document.createElement('div');
      time.className = 'col-sm-4 text-center';
      time.style.fontStyle = 'italic';
      time.innerHTML = 'TIME: ' +  moment(data[this.getProcedure()].lastObservation.date).format('HH:mm');
      ...
      data[this.getProcedure()].observedSpec.forEach((property) => {
         ...
         let imgUrl = WidgetFunctions.getUrlByValue(data[this.getProcedure()].lastObservation.properties[property.name], property.intervals);

         img.setAttribute('src', (imgUrl != undefined) ? imgUrl : 'https://live.osgeo.org/_images/logo-istsos.png');
         ...
         valueCol.innerHTML = parseFloat(data[this.getProcedure()].lastObservation.properties[property.name]).toFixed(2) + '&nbsp;' + property.uom;
